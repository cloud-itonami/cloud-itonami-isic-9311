(ns facility.operation
  "OperationActor -- one sports-facility operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (FacilityOps-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Facility Safety Governor
  ...
  One graph run = one sports-facility operation (intake -> advise ->
  ...
  decision to a human operator (the venue operator). The approver
  resumes with `{:approval {:status :approved}}` (or :rejected).
  `:facility/authorize-use` ALWAYS reaches this node when the governor
  is clean -- see `facility.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [facility.facilityopsllm :as facilityopsllm]
            [facility.governor :as governor]
...
(defn build
  ...
  opts:
    :advisor      -- a `facility.facilityopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (facilityopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; FacilityOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (facilityopsllm/-advise advisor store request)]
            {:proposal p :audit [(facilityopsllm/trace request p)]})))

      ;; Facility Safety Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
